...
rev.sim.NPID <- function(x, thr_ic_KS_rs = 1.7) {
  ...
  print("L_inf with IC")
  start_time <- Sys.time()
  z <- model_selection_L_inf(x, th_const = thr_ic_KS_rs, rescale = TRUE, points = 15)
  end_time <- Sys.time()
  cpt.z = z$cpt_ic
  
  Linf_RS_IC <- new("cpt.est")
  if(any(is.na(cpt.z))){Linf_RS_IC@cpt = 0
  Linf_RS_IC@nocpt = 0}
  else{Linf_RS_IC@cpt <- cpt.z
  Linf_RS_IC@nocpt <- length(cpt.z)}
  Linf_RS_IC@mean <- mean.from.cpt(x, Linf_RS_IC@cpt)
  Linf_RS_IC@time <- as.numeric(difftime(end_time,start_time, units = 'secs'))
...
rev.sim.comp <- function(x, ARL2 = NULL) {
  ...
  pelt.np@time <- as.numeric(difftime(end_time,start_time, units = 'secs'))
  ...
  CPM_points@time <- as.numeric(difftime(end_time,start_time, units = 'secs'))
  
  print("CPMKS2")
  start_time <- Sys.time()
  z <- processStream(x,cpmType = "Kolmogorov-Smirnov",ARL0 = ARL2)
  end_time <- Sys.time()
  ...
  CPM_points2@time <- as.numeric(difftime(end_time,start_time, units = 'secs'))
  ...
  CPM_points3@time <- as.numeric(difftime(end_time,start_time, units = 'secs'))
  
  print("CPMcvm2")
  start_time <- Sys.time()
  z <- processStream(x,cpmType = "Cramer-von-Mises",ARL0 = ARL2)
  end_time <- Sys.time()
  ...
  CPM_points4@time <- as.numeric(difftime(end_time,start_time, units = 'secs'))
  
  print("ecp")
  start_time <- Sys.time()
  z <- e.divisive(X=matrix(x, length(x), 1),alpha=1)
  end_time <- Sys.time()
  ecp <- new("cpt.est")
  if (length(z$estimates) == 2){
  ecp@cpt <- 0
  ecp@nocpt <- 0} else{
    ecp@cpt <- z$estimates[-c(1,length(z$estimates))]
    ecp@nocpt <- length(ecp@cpt)}
  ecp@time <- as.numeric(difftime(end_time,start_time, units = 'secs'))
  ...
  nbs@time <- as.numeric(difftime(end_time,start_time, units = 'secs'))
...
nwbs@nocpt <- length(nwbs@cpt)}
 nwbs@time <- as.numeric(difftime(end_time,start_time, units = 'secs'))
  
  list(pelt.np=pelt.np,CPM_points=CPM_points,CPM_points2=CPM_points2,
       CPM_points3=CPM_points3, CPM_points4=CPM_points4,ecp = ecp, 
       nbs = nbs, nwbs = nwbs)
}
...
rev.sim.study.no_cpt <- function(lx = NULL, m = 100, seed = NULL) {
...
rev.sim.study.no_cpt_comp <- function(lx, m = 100, seed = NULL) {
  ...
  for (i in 1:m) {
    ...
    est <- rev.sim.comp(x, ARL2 = ARL_max)
...
rev.sim.study.NPID.mean <- function(signal, noise = NULL, param = NULL, expon = FALSE, no.of.cpt, true.cpt=NULL, m = 100, seed = NULL, resc = FALSE) {
  ...
  for (i in 1:m) {
    ...
    print(i)
    x <- signal + param * rnorm(n)
    if(expon == TRUE){x <- exp(x)}
    est <- rev.sim.NPID(x)
...
## study for changes in the mean
rev.sim.study.comp_mean <- function(signal, noise = NULL, param = NULL, expon = FALSE, no.of.cpt, true.cpt=NULL, m = 100, seed = NULL) {
...
SIM_mean_1 <- rev.sim.study.NPID.mean(mean_test1, noise = "Gaussian", param = 1, expon = FALSE, seed = 12, m = 100, true.cpt = c(50), no.of.cpt = 1)

SIM_multi_mean_1_Gauss <- rev.sim.study.NPID.mean(multi_mean_test1, noise = "Gaussian", param = 1, expon = FALSE, seed = 12, m = 100, true.cpt = c(100, 200, 300), no.of.cpt = 3)

SIM_multi_mean_1_Gauss_exp <- rev.sim.study.NPID.mean(multi_mean_test1, noise = "Gaussian", param = 1, expon = TRUE, seed = 12, m = 100, true.cpt = c(100, 200, 300), no.of.cpt = 3)

SIM_multi_mean_1_Pois <- rev.sim.study.NPID.mean(multi_mean_test1, noise = "Poisson", param = 1, expon = FALSE, seed = 12, m = 100, true.cpt = c(100, 200, 300), no.of.cpt = 3)

SIM_multi_mean_1_Pois_exp <- rev.sim.study.NPID.mean(multi_mean_test1, noise = "Poisson", param = 1, expon = TRUE, seed = 12, m = 100, true.cpt = c(100, 200, 300), no.of.cpt = 3)

SIM_multi_mean_1_student <- rev.sim.study.NPID.mean(multi_mean_test1, noise = "Student", param = 1, expon = FALSE, seed = 12, m = 100, true.cpt = c(100, 200, 300), no.of.cpt = 3)

SIM_multi_mean_2_Gauss <- rev.sim.study.NPID.mean(multi_mean_test2, noise = "Gaussian", param = 1, expon = FALSE, seed = 12, m = 100, true.cpt = seq(80,1520,80), no.of.cpt = 19)

## Results for the competing methods
SIM_mean_1_comp <- rev.sim.study.comp_mean(mean_test1, noise = "Gaussian", param = 1, expon = FALSE, seed = 12, m = 100, true.cpt = c(100), no.of.cpt = 1)

SIM_multi_mean_1_comp_Gauss <- rev.sim.study.comp_mean(multi_mean_test1, noise = "Gaussian", param = 1, expon = FALSE, seed = 12, m = 100, true.cpt = c(100, 200, 300), no.of.cpt = 3)

SIM_multi_mean_1_comp_Gauss_exp <- rev.sim.study.comp_mean(multi_mean_test1, noise = "Gaussian", param = 1, expon = TRUE, seed = 12, m = 100, true.cpt = c(100, 200, 300), no.of.cpt = 3)

SIM_multi_mean_1_comp_Pois <- rev.sim.study.comp_mean(multi_mean_test1, noise = "Poisson", param = 1, expon = FALSE, seed = 12, m = 100, true.cpt = c(100, 200, 300), no.of.cpt = 3)

SIM_multi_mean_1_comp_Pois_exp <- rev.sim.study.comp_mean(multi_mean_test1, noise = "Poisson", param = 1, expon = TRUE, seed = 12, m = 100, true.cpt = c(100, 200, 300), no.of.cpt = 3)

SIM_multi_mean_1_comp_student <- rev.sim.study.comp_mean(multi_mean_test1, noise = "Student", param = 1, expon = FALSE, seed = 12, m = 100, true.cpt = c(100, 200, 300), no.of.cpt = 3)

SIM_multi_mean_2_comp_Gauss <- rev.sim.study.comp_mean(multi_mean_test2, noise = "Gaussian", param = 1, expon = FALSE, seed = 12, m = 100, true.cpt = seq(80,1520,80), no.of.cpt = 19)
...
## Results for our method
rev.sim.study.NPID.var1 <- function(lx = NULL, true.cpt = NULL, m = 100, seed = NULL, sv = NULL) {
...
rev.sim.study.NPID.var3 <- function(lx = NULL, true.cpt = NULL, m = 100, seed = NULL, sv = NULL) {
...
rev.sim.study.NPID.var5 <- function(lx = NULL, true.cpt = NULL, m = 100, seed = NULL, sv = NULL) {
...
SIM_multi_var_2 <- rev.sim.study.NPID.var5(lx = 1000, true.cpt = c(200,350,550,700,900), m = 100, seed = 12, sv = c(sqrt(10),sqrt(2),sqrt(0.3),2,sqrt(20),sqrt(2)))
...
rev.sim.study.comp.var1 <- function(lx, sv = NULL, true.cpt=NULL, m = 100, seed = NULL) {
  ...
  list(pelt.np=pelt.np,CPM_points=CPM_points,CPM_points2=CPM_points2,
       CPM_points3=CPM_points3, CPM_points4=CPM_points4, ecp = ecp, nbs = nbs, nwbs = nwbs)
}

SIM_var_1_comp <- rev.sim.study.comp.var1(lx = 500, true.cpt = c(250), m = 100, seed = 12, sv = 2)
...
SIM_multi_var_2_comp <- rev.sim.study.comp.var5(lx = 1000, true.cpt = c(200,350,550,700,900), m = 100, seed = 12, sv = c(sqrt(10),sqrt(2),sqrt(0.3),2,sqrt(20),sqrt(2)))
...
rev.sim.study.NPID.dist1 <- function(lx = NULL, true.cpt = NULL, m = 100, seed = NULL) {
  ...
  for (i in 1:m) {
    ...
    print(i)
    x <- c(runif(true.cpt[1],-3,3), rt(lx - true.cpt[1],3))
...
SIM_distr_1 <- rev.sim.study.NPID.dist1(1000, true.cpt = c(500), m= 100, seed = 12)

rev.sim.study.NPID.dist3 <- function(lx = NULL, true.cpt = NULL, m = 100, seed = NULL) {
...
SIM_distr_3 <- rev.sim.study.NPID.dist3(500, true.cpt = c(100,250,350), m= 100, seed = 12)

rev.sim.study.NPID.dist3_2 <- function(lx = NULL, true.cpt = NULL, m = 100, seed = NULL) {
...
SIM_distr_3_2 <- rev.sim.study.NPID.dist3_2(1000, true.cpt = c(200,500,750), m= 100, seed = 12)

rev.sim.study.NPID.dist3_3 <- function(lx = NULL, true.cpt = NULL, m = 100, seed = NULL) {
  ...
  for (i in 1:m) {
    ...
    print(i)
    x <- c(rgamma(true.cpt[1],1,1), rpois(true.cpt[2] - true.cpt[1],1), runif(lx - true.cpt[2],1 - sqrt(3),1+sqrt(3)))
...
SIM_distr_3_3 <- rev.sim.study.NPID.dist3_3(750, true.cpt = c(250,500), m= 100, seed = 12)

## Results for the competitors
rev.sim.study.comp.dist1 <- function(lx = NULL, true.cpt = NULL, m = 100, seed = NULL) {
...
SIM_distr_1_comp <- rev.sim.study.comp.dist1(1000, true.cpt = c(500), m= 100, seed = 12)
...
SIM_distr_3_comp <- rev.sim.study.comp.dist3(500, true.cpt = c(100,250,350), m= 100, seed = 12)
...
rev.sim.study.comp.dist3_3 <- function(lx = NULL, true.cpt = NULL, m = 100, seed = NULL) {
...
SIM_distr_3_comp <- rev.sim.study.comp.dist3_3(750, true.cpt = c(250,500), m= 100, seed = 12)
